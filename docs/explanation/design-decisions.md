...
The iOS client needs to cancel the in-flight URLSession task when the user navigates away from the scanning view (T10). `URLSession.bytes(for:)` — the simpler async/await streaming API — does not expose the underlying `URLSessionDataTask` directly, so there is no clean way to cancel it from a different call site. `URLSessionDataDelegate` stores the task at construction time (`IOSScanSession.dataTask`) and exposes a `cancel()` method, which is why the iOS client uses the delegate API despite its extra boilerplate.
