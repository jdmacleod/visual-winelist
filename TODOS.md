...
- **chore/version-sync-guard** — Version consistency across clients (via /investigate). The version lived in seven stores (`VERSION`, `backend/pyproject.toml`, `backend/uv.lock`, `web/package.json`, `web/package-lock.json`, iOS `MARKETING_VERSION`, README badge) but only `VERSION` was bumped on release, so the rest drifted — iOS reported `0.2.12` for eleven releases and stamped every opt-in scan-telemetry row with that stale `app_version`. Backend `/health` was always correct (reads `VERSION` at runtime). Added `Scripts/version_sync.py` (`set` propagates to all seven with anchored regexes + 3/4-part handling; `check` asserts agreement), `Scripts/bump-version.sh` wrapper, a `version-sync` CI job, and `backend/tests/test_version_sync.py` (6 tests: live invariant + write engine + validation + dispatch). Synced all stores and rewrote the `CONTRIBUTING.md` release steps. **Completed:** v0.3.4.0 (2026-06-25)
- **fix/scan-save-reliability-and-live-notes** — Three field-reported issues (via /investigate). (1) Scan-image saves were coupled to SSE stream consumption inside `_scan_sse` after the first `: ready` byte, so an early client disconnect (iOS cancels the URLSession on view dismiss / re-scan) dropped the photo; moved persistence to a detached task scheduled in the `/scan` handler (`_schedule_scan_save`), awaited in the generator's `finally` for determinism, with failures logged via `_on_save_done` instead of swallowed. (2) `WineDetailView` rendered a value snapshot captured at navigation time, so streamed tasting notes never reached the open card; it now resolves live `WineState` from the `@Observable` view model by `wine.id`. (3) Added a wine-red notes-ready outline to `WineBottleCard` alongside the existing badge. 4 new backend tests (early-disconnect persistence, save-failure logging, cancelled-task callback) + 1 iOS contract test (id-stable note resolution). **Completed:** v0.3.3.0 (2026-06-25)
