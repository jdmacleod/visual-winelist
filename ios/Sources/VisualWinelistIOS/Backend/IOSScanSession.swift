...
///
/// Scan timing is captured here unconditionally (not just #if DEBUG): it feeds the
/// opt-in telemetry reporter in Release as well as the debug HUD. Only the SEND of
/// telemetry is gated by the user's "Send Diagnostics?" preference.
...
final class IOSScanSession: NSObject, URLSessionDataDelegate, @unchecked Sendable {
    ...
    // can detect they're stale before writing into the next scan's metrics.
    ...
    func cancel() {
        // Only a real interruption (user cancel / view dismiss mid-scan) starts a new
        // generation. Normal completion ALSO routes through here via
        // continuation.onTermination, but by then the task is already `.completed` —
        // bumping the generation there would wrongly drop this scan's own end-of-task
        // transaction metrics (didFinishCollecting fires at completion, and its
        // MainActor write is gen-guarded). Snapshot state before cancel().
    ...
    func urlSession(
        ...
        case 200:
            let scanId = http.value(forHTTPHeaderField: "X-Scan-Id")
            if let t0 = debugT0 {
                ...
                Task { @MainActor [weak self] in
                    ...
                    if let scanId { DebugStore.shared.recordScanId(scanId) }
                }
            }
