    ...
    public final class StateServer {
        ...
        public func start() {
            ...
            logger.notice(
                ...
                "gstack-ios-qa-bootstrap token=\(self.bootToken, privacy: .public) port=\(self.port, privacy: .public) build=\(self.appBuildId, privacy: .public)"
            )
        ...
        public func registerAccessor(
            key: String, type: String, read: @escaping ReadHandler, write: @escaping WriteHandler
        ) {
        ...
        private func startListener(family: AddressFamily) {
            do {
                ...
                listener.stateUpdateHandler = { [weak self] state in
                    Task { @MainActor in
                        if case .ready = state {
                            ...
                            self?.logger.error(
                                "StateServer listener failed: \(err.localizedDescription, privacy: .public)")
                        }
                    }
                }
                ...
                logger.error(
                    "Listener bind failed (\(String(describing: family))): \(error.localizedDescription, privacy: .public)"
                )
            }
        }
        ...
        private func handleAuthRotate(connection: NWConnection, request: ParsedRequest) {
            ...
            guard bootTokenValid,
                ...
                String(auth.dropFirst("Bearer ".count)) == bootToken
            else {
        ...
        private func handleStateWrite(connection: NWConnection, request: ParsedRequest, key: String) {
            ...
            if ok {
                ...
                send(
                    connection: connection, status: 400,
                    body: ["error": "type_mismatch", "expected": typeNames[key] ?? "?"])
            }
        }
        ...
        private func handleSnapshotRestore(connection: NWConnection, request: ParsedRequest) {
            ...
            if let hash = envelope["_accessor_hash"] as? String, hash != accessorHash {
                send(
                    connection: connection, status: 409,
                    body: [
            ...
            case .missingKey(let k):
                send(
                    connection: connection, status: 400,
                    body: ["error": "validation_failed", "key": k, "reason": "missing"])
            case .typeMismatch(let k):
                send(
                    connection: connection, status: 400,
                    body: ["error": "validation_failed", "key": k, "reason": "type-mismatch"])
            case .schemaMismatch(let expected, let got):
                send(
                    connection: connection, status: 409,
                    body: ["error": "schema_mismatch", "expected_hash": expected, "got_hash": got])
            }
        }
