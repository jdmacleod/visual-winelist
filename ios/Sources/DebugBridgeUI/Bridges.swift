...
//   #endif
    ...
    enum ElementsBridgeImpl {
        ...
        private static func collect(
            view: UIView, parentPath: String, windowBounds: CGRect, into elements: inout [JSONDict]
        ) {
    ...
    enum MutationBridgeImpl {
        ...
        /// Tap at (x, y) in window coordinates. Hit-tests the window to find the
        /// target view, sends UIControl actions for interactive controls, and falls
        /// back to accessibilityActivate() for other views (covers SwiftUI Buttons
        /// that surface as accessibility elements).
        ...
        private static func handleSwipe(_ payload: JSONDict) -> Bool {
            guard let fx = payload["from_x"] as? NSNumber,
                ...
                let ty = payload["to_y"] as? NSNumber
            else { return false }
